...
public:
    CubebPortBackend(cubeb* ctx, const PortOut& port)
        : frame_size(port.format_info.FrameSize()),
          buffer(static_cast<int>(port.BufferSize()) * 4) {
        if (!ctx) {
            return;
        }
        const auto get_channel_layout = [&port] -> cubeb_channel_layout {
            switch (port.format_info.num_channels) {
            case 1:
        ...
        cubeb_stream_params stream_params = {
            .format = port.format_info.is_float ? CUBEB_SAMPLE_FLOAT32LE : CUBEB_SAMPLE_S16LE,
